...
## bump: rewrite the mechanical release version sites (VERSION=x.y.z BUILD=n)
bump:
	@[ -n "$(VERSION)" ] && [ -n "$(BUILD)" ] || { echo "usage: make bump VERSION=x.y.z BUILD=n"; exit 2; }
	./scripts/bump-version.sh "$(VERSION)" "$(BUILD)"

## bump-check: verify the version bump rewrites against the real file shapes
bump-check:
	./scripts/bump-version.sh --self-test

## project-version-check: verify the single MARKETING_VERSION parser
project-version-check:
	./scripts/project-version.sh --self-test
...
## commit-ci-check: validate the release gate's reading of a commit's CI check runs
commit-ci-check:
	python3 scripts/verify-commit-ci.py --self-test
...
lint: sanitizer-check hygiene project-version-check bump-check perf-check coverage-check swift-features-check build-boundaries-check informational-update-check release-promotion-check commit-ci-check qa-handoff-check memory-smoke-check ui-test-preflight-check screenshot-tour-check
