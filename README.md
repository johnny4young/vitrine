...
[![Status](https://img.shields.io/badge/status-v0.8.1%20shipped-brightgreen.svg)](#status)
...
- 🖥️ **Terminal output → image** — paste or drop colored terminal output (`git`, test runners, build logs) and Vitrine renders the ANSI/SGR styling (16 / 256 / truecolor, bold · italic · underline · strikethrough · inverse); the palette follows your theme. The `vgrab` / `vlast` shell helpers *(PRO)* capture a command's output with its color intact. → [`docs/TERMINAL.md`](docs/TERMINAL.md).
...
🟢 **v0.8.1 — shipped and stable.** Everything under [Features](#features) is built and
