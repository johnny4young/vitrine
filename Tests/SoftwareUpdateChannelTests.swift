...
/// CS-064 — Sparkle auto-update channel.
///
/// These tests assert that the committed configuration, resources, workflow, and docs
/// actually encode every acceptance criterion of the ticket, so a future edit that drops
...
///
/// Like `WorkflowConfigurationTests` (CS-060), `ReleaseSigningTests` (CS-061),
/// `AppStoreReadinessTests` (CS-062), and the permission-matrix suites (CS-065), they read
...
/// runner.
@Suite("Software update channel · CS-064")
struct SoftwareUpdateChannelTests {
    ...
    /// The minimal app-target entitlements (App Store / Phase 1 / default build).
    ...
    @Test func theUpdateChannelFilesExist() {
        ...
        for path in [
            ...
            #expect(
                ...
                "CS-064 expects \(path.lastPathComponent) to exist")
        }
    }
    ...
    @Test func cliExcludesSparkleViaCompilationConditions() throws {
        ...
        #expect(
            ...
            "the CLI target must drop VITRINE_DIRECT_DOWNLOAD so it never links Sparkle (CS-064)")
    }

    // MARK: - Acceptance: SUFeedURL points to a signed appcast (over HTTPS)
    ...
    @Test func infoPlistDeclaresAnHTTPSAppcastFeedURL() throws {
        ...
        let feed = try #require(
            ...
            "Info.plist must declare SUFeedURL for the appcast (CS-064)")
        #expect(
    ...
    @Test func infoPlistDeclaresAnEdDSAPublicKeyForSignatureVerification() throws {
        ...
        let key = try #require(
            ...
            "Info.plist must declare SUPublicEDKey so Sparkle verifies update signatures (CS-064)")
        #expect(!key.isEmpty, "SUPublicEDKey must not be empty")
    }

    // MARK: - Acceptance: update checks do not collect analytics
    ...
    @Test func systemProfilingIsDisabledSoUpdateChecksCollectNoAnalytics() throws {
        ...
        #expect(
            ...
            "SUEnableSystemProfiling must be NO so update checks collect no analytics (CS-064)")
        ...
        #expect(
            ...
            "SoftwareUpdater.swift must document the no-analytics posture (CS-064)")
    }

    // MARK: - Acceptance: update checks are user-visible
    ...
    @Test func updateCheckIsUserVisibleThroughACommand() throws {
        ...
        #expect(
            ...
            "a user-visible Check for Updates command must exist (CS-064)")
        #expect(
    ...
    // MARK: - Acceptance: appcast is published with each GitHub release
    ...
    @Test func releaseWorkflowPublishesTheAppcastWithEachRelease() throws {
        ...
        #expect(
            ...
            "release.yml must generate the appcast with Sparkle's generate_appcast (CS-064)")
        #expect(
        ...
        #expect(
            ...
            "release.yml must sign the appcast with the EdDSA private-key secret (CS-064)")
        ...
        #expect(
            ...
            "the published GitHub release must attach the appcast (CS-064)")
        #expect(
            release.contains("deploy-pages") || release.contains("upload-pages-artifact"),
            "release.yml must deploy the appcast to GitHub Pages (the SUFeedURL host) (CS-064)")
    }
    ...
    @Test func appcastFeedURLMatchesThePagesDeployTarget() throws {
        ...
        #expect(
            ...
            "SUFeedURL must be the GitHub Pages feed the release workflow deploys (CS-064)")
        let release = try Self.release()
        #expect(
            release.contains("deploy-pages"),
            "release.yml must deploy the appcast to GitHub Pages so SUFeedURL resolves (CS-064)")
    }

    // MARK: - Acceptance: App Store build excludes Sparkle
    ...
    @Test func appStoreBuildExcludesSparkleViaCompilationFlagAndStrip() throws {
        ...
        #expect(
            ...
            "project.yml must define VITRINE_DIRECT_DOWNLOAD for the direct-download build (CS-064)"
        )
        ...
        #expect(
            ...
            "the App Store archive must remove VITRINE_DIRECT_DOWNLOAD so Sparkle compiles out (CS-064)"
        )
        #expect(
            appStore.contains("Sparkle.framework"),
            "the App Store archive must strip the Sparkle framework (CS-064)")
        ...
        #expect(
            ...
            "the App Store workflow must verify Sparkle is excluded from the archive (CS-064)")
    }
    ...
    // MARK: - Acceptance: EdDSA signing keys are generated and documented
    ...
    /// mach-lookup exceptions. The minimal app entitlements (App Store / Phase 1) must stay
    /// free of both, so the App Store posture and the Phase 1 "no network" guarantee are
    /// unchanged.
    @Test func directDownloadEntitlementsGrantSparkleNetworkAndXPCButTheMinimalSetDoesNot() throws {
        ...
        #expect(
            ...
            "the direct-download entitlements must grant network for Sparkle downloads (CS-064)")
        ...
        #expect(
            ...
            "the direct-download entitlements must include Sparkle's XPC mach-lookup exceptions (CS-064)"
        )

        // The minimal (App Store / Phase 1) entitlements must NOT carry network or the Sparkle
        ...
        #expect(
            ...
            "the App Store / Phase 1 entitlements must stay network-free (CS-064/CS-065)")
        #expect(
            minimal["com.apple.security.temporary-exception.mach-lookup.global-name"] == nil,
            "the App Store / Phase 1 entitlements must not carry Sparkle's XPC exceptions (CS-064)")
    }
    ...
    @Test func releasingDocDocumentsKeysAppcastAndChannelExclusion() throws {
        ...
        #expect(
            ...
            "RELEASING.md must document generating the EdDSA keys with generate_keys (CS-064)")
        #expect(
            doc.contains("SPARKLE_EDDSA_PRIVATE_KEY"),
            "RELEASING.md must document the EdDSA private-key secret (CS-064)")
        #expect(
            doc.contains("SUPublicEDKey"),
            "RELEASING.md must document pasting the public key into Info.plist (CS-064)")
        ...
        #expect(
            ...
            "RELEASING.md must document the appcast (CS-064)")
        #expect(
            doc.localizedCaseInsensitiveContains("N to N+1")
                || doc.localizedCaseInsensitiveContains("N+1"),
            "RELEASING.md must document testing an update from N to N+1 (CS-064)")
        #expect(
            doc.localizedCaseInsensitiveContains("signature"),
            "RELEASING.md must document signature verification (CS-064)")
        ...
        #expect(
            ...
            "RELEASING.md must document that the App Store build excludes Sparkle (CS-064)")
        ...
        #expect(
            ...
            "RELEASING.md must state the update path collects no analytics (CS-064)")
    }
}
