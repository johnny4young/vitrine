...
public enum ColorProfile: String, CaseIterable, Identifiable, Codable, Sendable {
    case sRGB
    case displayP3
    ...
    public static let fallback: ColorProfile = .sRGB

    /// Decodes a persisted raw value, tolerating `nil` or an unrecognized
    /// string by returning `fallback` (documented fallback).
    public static func resolve(_ rawValue: String?) -> ColorProfile {
