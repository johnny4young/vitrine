...
///
/// This lives in VitrineCLICore rather than in `main.swift`, so the executable is a single
/// call and everything it runs is compiled once, into the library the tests link too. The
/// module's MainActor-default isolation puts `run()` on the main actor, where the
/// `@MainActor` renderer is called directly.
public enum CLICommandLine {
    /// Runs the invocation in `CommandLine.arguments` and exits with its status.
    public static func run() -> Never {
        ...
        if rawArguments.first == "recipe" {
            ...
            case .unknownAction(let action):
                printError(
                    "error: unknown recipe action \"\(action)\".\n\n" + CLIRecipeCommand.usage)
                exit(2)
            case .unknownFlag(let flag):
        ...
        do {
            ...
            // terminal-capture and render --edit never read a PRO token,
