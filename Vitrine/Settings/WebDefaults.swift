...
enum WebDefaults {
    /// The same key constants the writer uses (`WebCaptureSettings`). Reading through
    /// them keeps both halves of each setting renameable in one place: a literal here
    /// would let a renamed key compile while silently reverting the setting to its
    ...
    {
        guard let raw = defaults.string(forKey: Keys.webViewportKind),
    ...
    static func viewports(
        ...
        guard let raw = defaults.array(forKey: Keys.webViewports) as? [String] else {
    ...
    static func customViewportWidth(from defaults: UserDefaults) -> Int {
        guard let value = defaults.object(forKey: Keys.webCustomViewportWidth) as? Int else {
    ...
    static func customViewportHeight(from defaults: UserDefaults) -> Int {
        guard let value = defaults.object(forKey: Keys.webCustomViewportHeight) as? Int else {
    ...
    static func captureMode(from defaults: UserDefaults) -> WebSnapshotConfig.CaptureMode {
        guard let raw = defaults.string(forKey: Keys.webCaptureMode),
    ...
    static func waitKind(from defaults: UserDefaults) -> WebSnapshotConfig.WaitStrategy.Kind {
        guard let raw = defaults.string(forKey: Keys.webWaitKind),
    ...
    static func waitSeconds(from defaults: UserDefaults) -> Int {
        guard let value = defaults.object(forKey: Keys.webWaitSeconds) as? Int else {
            return waitSeconds
        }
