...
/// The code renderer: turns a `.code` input into a framed, syntax-highlighted
/// image. It is a thin, network-free wrapper over the existing render
...
/// independently of the web capture web paths.
///
/// The renderer reads only the snapshot configuration it is handed; it never
/// touches the network, the clipboard, or any URL configuration, which the contract
/// tests assert.
struct CodeRenderer: Renderer {
    ...
    /// 1200×630). `nil` lets the canvas size to its content.
