...
import VitrineRendering
    ...
    final class AppLaunchArgumentHandler {
        ...
        func handle(_ arguments: [String] = ProcessInfo.processInfo.arguments) -> Bool {
            ...
            if arguments.contains("--demo-beautify-image") {
                ...
                EditorWindowController.shared.show()
                // A real compact frame is more deterministic than dragging a hosted
                // window edge with XCUITest. The size assertion remains in the UI test,
                // so a content minimum that rejects this viewport still fails the test.
                    ...
                    do {
                        for tick in 0..<iterations {
                            ...
                            guard Self.snapshotOpenWindows(tag: tick) != nil else {
                                throw MemoryLargeDocumentCycleJourney.JourneyError
    ...
    /// Release ships no launch hooks.
    ///
    /// Everything this type does is development and release-QA scaffolding: seeding demo
    /// content, forcing windows open past their gates, pinning appearance, and — the one
    /// that matters most — swapping in a fake Web Snapshot renderer. A signed, notarized
    /// build responded to all of it, because the only condition guarding it was
    /// `VITRINE_GUI`, which Release defines.
    ///
    /// Every consumer is a Debug build: the UI suite and the screenshot tour build Debug,
