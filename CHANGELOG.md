...
- **Release tooling is reproducible and auditable.** CI verifies the exact XcodeGen
  version, watches pinned external tools for upstream releases, validates App Store
  archives through Xcode instead of deprecated delivery tooling, and publishes an SPDX
  dependency inventory with every GitHub release.
